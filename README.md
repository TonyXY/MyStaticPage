MyStaticPagede是在StaticPagede基础上修改的 静态页面自动化工具
==========
...
go.sh脚本，直接在终端运行以下命令就可以一键(1.git clone 2.重命名 3.修改项目配置 等)配置好所有的文件，不用再一个一个打开文件修改文件。脚本为`go.sh`文件，OSX系统下`sh go.sh`即可执行，大家看下就能理解。

````
git clone https://github.com/TonyXY/MyStaticPage.git && cd StaticPage && sh go.sh
````
...
````
git clone https://github.com/TonyXY/MyStaticPage.git
````
